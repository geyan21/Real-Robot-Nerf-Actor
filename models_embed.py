...
from agents.nerfodiseactv4_bc.resnetfc import ResnetFC
import torch.autograd.profiler as profiler
from agents.nerfodiseactv4_bc.utils import repeat_interleave
import os
...
class PixelNeRFEmbedNet(torch.nn.Module):
    def __init__(self, conf, coordinate_bounds, stop_encoder_grad=False):
        """
        ...
        print(colored(f"[PixelNeRFEmbedNet] coordinate_bounds: {self.coordinate_bounds}", "red"))
        # for voxel sampling
        ...
        self.canon_xyz = True
        print(colored(f"[PixelNeRFEmbedNet] canon_xyz: {self.canon_xyz}", "red"))
        ...
        print(colored(f"[PixelNeRFEmbedNet] regress_coord: {self.regress_coord}", "red"))

        # Regress attention
        self.regress_attention = conf.regress_attention
        print(colored(f"[PixelNeRFEmbedNet] regress_attention: {self.regress_attention}", "red"))
        ...
        cprint(f"[PixelNeRFEmbedNet] use_multi_scale_voxel: {self.use_multi_scale_voxel} with d_latent: {self.d_latent}", "red")


        
        self.use_depth_supervision = conf.use_depth_supervision
        cprint(f"[PixelNeRFEmbedNet] use_depth_supervision: {self.use_depth_supervision}", "red")
        ...
        self.mlp_coarse = ResnetFC(d_in=d_in, d_latent=d_latent, d_lang=d_lang, d_out=d_out, 
                                    ...
                                    beta=conf.mlp.beta, use_spade=conf.mlp.use_spade,use_language=conf.mlp.use_language,)
        if self.share_mlp:
    ...
    def encode(self, voxel_feat, lang, multi_scale_voxel_list, voxel_density, poses, focal,  c=None):
        """
        ...
        if not self.use_depth_supervision:
            self.voxel_density = None
        self.language = lang # B x L x 128
        ...
        self.focal[..., 1] *= -1.0
        self.focal.to(self.voxel_feat.device)

        if c is None:
            # Default principal point is center of image
            # c = (self.image_shape[0] * 0.5).unsqueeze(0)
            c = None    
        elif len(c.shape) == 0:
    ...
    def sample_in_voxel(self, xyz):
        ...
        bb_mins = coord_bounds[..., 0:3].to(xyz.device)
        bb_maxs = coord_bounds[..., 3:6].to(xyz.device)
        bb_ranges = bb_maxs - bb_mins
        MIN_DENOMINATOR = 1e-12
        res = bb_ranges / (self._dims_orig.float().to(xyz.device) + MIN_DENOMINATOR)
        voxel_indicy_denmominator = res + MIN_DENOMINATOR
        
        # align point cloud to voxel grid
        bb_mins_shifted = bb_mins - self._res.to(xyz.device)
        xyz_voxel_space -= bb_mins_shifted
        xyz_voxel_space /= voxel_indicy_denmominator
        # # scaling for the voxel grid
        # xyz_voxel_space *= scale # scale to feature voxel
    ...
    def forward(self, xyz, coarse=True, viewdirs=None, far=False, ret_last_feat=False):
        ...
        with profiler.record_function("model_inference"):
            ...
            USE_CANONICAL_COORD = True

            if USE_CANONICAL_COORD:
                # normalize the point cloud bounding box into [0, 1]
            ...
            if use_volume:
                if USE_CANONICAL_COORD:
                    ...
                    latent, point_density = self.sample_in_voxel(canon_xyz) # (B, N, Feat)
            ...
            #print(coarse, prev_output.shape)
