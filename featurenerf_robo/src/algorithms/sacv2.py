...
class SACv2(object):
    def __init__(self, obs_shape, state_shape, action_shape, args):
        ...
        args.observation_type = "state" if args.embedding_name != "none" else args.observation_type
    ...
    def behavior_clone_params(self):
        params = itertools.chain(self.encoder.parameters(), self.actor.parameters())
        return params
        
    def forward(self, obs, state=None):
        """
        simple forward pass for behavior cloning
        """
        obs = torch.FloatTensor(obs[:, :3]).div(255).cuda()
        state = torch.FloatTensor(state).cuda()
        obs = self.encoder(obs)
        mu, pi, log_pi, log_std = self.actor(obs, state)
