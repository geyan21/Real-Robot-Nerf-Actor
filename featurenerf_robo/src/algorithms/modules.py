...
class ContinuousPolicy(nn.Module):
    def __init__(self, input_dim:int, action_dim:int, state_dim:int=None, projection_dim=256 , hidden_dim=256, hidden_dim_state=128):
        super().__init__()

        self.trunk = nn.Sequential(nn.Linear(input_dim, projection_dim),
        ...
        if state_dim:
            self.state_encoder = nn.Sequential(nn.Linear(state_dim, hidden_dim_state),
    ...
    def forward(self, x, state=None, compute_pi=True, compute_log_pi=True):
        x = self.trunk(x)

        if self.state_encoder and state is not None:
...
class GripperPolicy(nn.Module):
    """
    predict continous translation and discrete gripper action
    By default we use a 3D gripper
    Thus the action space is (x, y, z, gripper) 4D
    """
    def __init__(self, input_dim:int, action_dim:int, state_dim:int=None, projection_dim=256 , hidden_dim=256, hidden_dim_state=128):
        super().__init__()
        ...
        self.space_predictor = nn.Linear(hidden_dim, 3) # continous 3D translation
        self.gripper_predictor = nn.Linear(hidden_dim, 2) # [open, close]
    ...
    def forward(self, x, state=None, compute_pi=True, compute_log_pi=True):
        ...
        # activation function
        xyz = self.space_predictor(h) # no activation function
        gripper = torch.softmax(self.gripper_predictor(h), dim=-1)
