...
def rgb2hsv_pairwise(rgb1, rgb2, eps=1e-8):
    return rgb2hsv(rgb1, eps), rgb2hsv(rgb2, eps)
def hsv2rgb(hsv):
...
def hsv2rgb_pairwise(hsv1, hsv2):
...
class ColorJitterPairwise(nn.Module):
    def __init__(self, brightness=0, contrast=0, saturation=0, hue=0, batch_size=128, stack_size=3):
        super(ColorJitterPairwise, self).__init__()
        self.brightness = self._check_input(brightness, 'brightness')
        self.contrast = self._check_input(contrast, 'contrast')
        self.saturation = self._check_input(saturation, 'saturation')
        self.hue = self._check_input(hue, 'hue', center=0, bound=(-0.5, 0.5),
    ...
    def adjust_contrast(self, x1, x2):
        """
            Args:
                x: torch tensor img (rgb type)
            Factor: torch tensor with same length as x
        ...
        factor = torch.empty(self.batch_size, device=_device).uniform_(*self.contrast)
        factor = factor.reshape(-1,1).repeat(1, self.stack_size).reshape(-1)
        means1 = torch.mean(x1, dim=(2, 3), keepdim=True)
        means2 = torch.mean(x2, dim=(2, 3), keepdim=True)
        return torch.clamp((x1 - means1) * factor.view(len(x1), 1, 1, 1) + means1, 0, 1) , \
                torch.clamp((x2 - means2) * factor.view(len(x2), 1, 1, 1) + means2, 0, 1)

    
    def adjust_hue(self, x1, x2):
        _device = x1.device
        factor = torch.empty(self.batch_size, device=_device).uniform_(*self.hue)
        factor = factor.reshape(-1,1).repeat(1, self.stack_size).reshape(-1)
        h = x1[:, 0, :, :]
        h += (factor.view(len(x1), 1, 1) * 255. / 360.)
        h = (h % 1)
        x1[:, 0, :, :] = h

        h = x2[:, 0, :, :]
        h += (factor.view(len(x2), 1, 1) * 255. / 360.)
        h = (h % 1)
        x2[:, 0, :, :] = h
        return x1, x2
    ...
    def adjust_brightness(self, x1, x2):
        ...
        factor = torch.empty(self.batch_size, device=_device).uniform_(*self.brightness)
        factor = factor.reshape(-1,1).repeat(1, self.stack_size).reshape(-1)
        x1[:, 2, :, :] = torch.clamp(x1[:, 2, :, :] * factor.view(len(x1), 1, 1), 0, 1)
        x2[:, 2, :, :] = torch.clamp(x2[:, 2, :, :] * factor.view(len(x2), 1, 1), 0, 1)

        return torch.clamp(x1, 0, 1), torch.clamp(x2, 0, 1)
    
    def adjust_saturate(self, x1, x2):
        """
            Args:
                x: torch tensor img (hsv type)
            Factor:
        ...
        factor = torch.empty(self.batch_size, device=_device).uniform_(*self.saturation)
        factor = factor.reshape(-1,1).repeat(1, self.stack_size).reshape(-1)
        x1[:, 1, :, :] = torch.clamp(x1[:, 1, :, :] * factor.view(len(x1), 1, 1), 0, 1)
        x2[:, 1, :, :] = torch.clamp(x2[:, 1, :, :] * factor.view(len(x2), 1, 1), 0, 1)
        return torch.clamp(x1, 0, 1), torch.clamp(x2, 0, 1)
    
    def transform(self, inputs1, inputs2):
        hsv_transform_list = [rgb2hsv_pairwise, self.adjust_brightness,
                              self.adjust_hue, self.adjust_saturate,
                              hsv2rgb_pairwise]
        rgb_transform_list = [self.adjust_contrast]
        # Shuffle transform
        if random.uniform(0,1) >= 0.5:
            ...
            transform_list = hsv_transform_list + rgb_transform_list
        for t in transform_list:
            inputs1, inputs2 = t(inputs1, inputs2)
        return inputs1, inputs2
    
    def forward(self, inputs1, inputs2):
        return self.transform(inputs1, inputs2)
