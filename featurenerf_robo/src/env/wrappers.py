...
def make_env(
    ...
    if domain_name == 'robot':
        ...
        env = FrameStack(env, frame_stack)
        env = DynamicCameraWrapper(env, domain_name=domain_name, camera_move_range=camera_move_range, \
                            num_static_cameras=num_static_cameras, seed=seed)
        if domain_randomization:
            raise NotImplementedError("Domain randomization not implemented for metaworld")
        env = CameraPosWrapper(env)
    elif domain_name == "dmc":
        domain, task = task_name.replace('-', '_').split('_', 1)
        env = suite.load(domain,
                        task,
                        task_kwargs={'random': seed},
                        visualize_reward=False)
        env = DMCWrapper(env, domain, task, cfg)
        if embedding_model is not None:
            env = EmbeddingWrapper(env, embedding_model)
        env = FrameStack(env, k=3) # hardcode
        env = FormatAlignWrapper(env)
...
class DMCWrapper(gym.Wrapper):
    def __init__(self, env, domain, task, cfg):
        ...
        env.metadata = None
        self.use_true_state = cfg.embedding_name == 'true_state'
        ...
        # modality
        self.modality = cfg.observation_type
        ...
        # finite horizon
        self.t = 0
        self.max_episode_steps = 1000 # hardcode

        # action repeat
        self.action_repeat = 2 # hardcode

        # action space
        wrapped_action_spec = env.action_spec()
        self.action_spec = specs.BoundedArray(wrapped_action_spec.shape,
                                                np.float32,
                                                wrapped_action_spec.minimum,
                                                wrapped_action_spec.maximum,
                                                'action')
        act_shp = self.action_spec.shape
        ...
        if self.modality == 'image':
            self.observation_space = gym.spaces.Box(low=0, high=255, shape=(3, self.image_size, self.image_size), dtype=np.uint8)
        elif self.modality == 'state':
            obs_shp = []
            ...
            self.observation_space = gym.spaces.Box( low=np.full(obs_shp, -np.inf), high=np.full(obs_shp, np.inf), shape=obs_shp, dtype=np.float32)
    ...
    def reset(self):
        ...
        state = np.concatenate([v.flatten() for v in time_step.observation.values()])
        self._true_state = state
        return self.get_obs(), None

    def get_obs(self):
        if self.modality == 'image':
            # torchvision.utils.save_image(torch.from_numpy(self.render(self.image_size)).float().div(255).permute(2,0,1),"obs.png")
            obs = self.render(width=self.image_size, height=self.image_size).transpose(2,0,1) # (H, W, C) -> (C, H, W)
            return obs
        elif self.modality == 'state':
    ...
    def render_obs(self, width=256, height=256):
        img = self.render(width=width, height=height)
        # extend to [1, 3, H, W]
    ...
    def step(self, action):
        ...
        state = np.concatenate([v.flatten() for v in time_step.observation.values()])
        self._true_state = state
        info = dict(is_success=0.) # do not have success in DMC
        
        return self.get_obs(), None, reward, time_step.last() or self.t == self.max_episode_steps, info
