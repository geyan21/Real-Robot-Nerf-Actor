...
class ResnetFC(nn.Module):
    def __init__(
        ...
        self.embed_out = None if d_embed == 0 else nn.Linear(d_hidden, d_embed) # embedding output
        ...
        if self.embed_out is not None:
            ...
            nn.init.kaiming_normal_(self.embed_out.weight, a=0, mode="fan_in")
    ...
    def forward(self, zx, combine_inner_dims=(1,), combine_index=None, dim_size=None):
        ...
        with profiler.record_function("resnetfc_infer"):
            ...
            embed = None if self.embed_out is None else self.embed_out(self.activation(x)) # embedding output
            return out, embed
