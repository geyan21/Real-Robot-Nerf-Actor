...
from representations.utils_pixelnerf.util import repeat_interleave
import os
...
class PixelNeRFEmbedNet(torch.nn.Module):
    def __init__(self, conf, stop_encoder_grad=False):
        ...
        self.use_encoder = conf.use_encoder  # Image features?
        ...
        self.use_code_viewdirs = conf.use_code_viewdirs  # Positional encoding applies to viewdirs

        # Enable view directions
        self.use_viewdirs = conf.use_viewdirs

        # Global image features?
        self.use_global_encoder = conf.use_global_encoder
        ...
        with profiler.record_function("model_inference"):
            ...
            if self.d_in > 0:
                ...
                if self.use_viewdirs:
                    ...
                    assert viewdirs is not None, "Must provide viewdirs (zero is fine)"
            ...
            if coarse or self.mlp_fine is None:
                mlp_output, embed_output = self.mlp_coarse(
                    mlp_input,
                ...
                mlp_output, embed_output = self.mlp_fine(
                    mlp_input,
                    combine_inner_dims=(self.num_views_per_obj, B),
                    combine_index=combine_index,
                    dim_size=dim_size,
            ...
            embed_output = embed_output.reshape(-1, B, self.d_embed) if embed_output is not None else None
    ...
    def load_weights(self, args, opt_init=False, strict=True, device=None):
        ...
        if os.path.exists(model_path):
            ...
            try:
                ...
                print("Warning: could not load all weights. load net with strict=False")
        elif not opt_init:
            warnings.warn(
                (
