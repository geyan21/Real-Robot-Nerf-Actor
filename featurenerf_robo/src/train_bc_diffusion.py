...
from diffusion_bc.policy import DiffusionPolicy
...
def evaluate(env, embedding_model, policy, L, step, args):
    num_episodes = 20
    success_rate = []
    returns = []
...
def collect_fn(batch):
	...
	for i in range(len(batch)):
		pre_obs, pre_state, action, reward, obs, state, done, info = batch[i]
		pre_obses.append(pre_obs)
		...
		dones.append(done)
		infos.append(info)
	pre_obses = np.stack(pre_obses)
	pre_states = np.stack(pre_states)
	...
	return dict(pre_obses=pre_obses, pre_states=pre_states, actions=actions, rewards=rewards, obses=obses, states=states, dones=dones, infos=infos)
...
def main(cfg):
    ...
    data_loader = DataLoader(dataset=bc_dataset, 
                            ...
                            collate_fn=collect_fn, shuffle=True, num_workers=4, drop_last=False)
    ...
    for s in range(cfg.num_seeds):
        ...
        policy = DiffusionPolicy(state_dim=embedding_model.output_dim, action_dim=action_dim[0])
        ...
        for epoch in range(cfg.num_epochs):
            ...
            for batch_idx, batch in enumerate(data_loader):
                ...
                obs = batch['pre_obses']
                state = batch['pre_states']
                action = batch['actions']
                reward = batch['rewards']
                next_obs = batch['obses']
                next_state = batch['states']
                done = batch['dones']
                info = batch['infos']
                ...
                for minibatch_idx in range(0, obs.shape[0], cfg.batch_size):
                    ...
                    # predict action
                    loss = policy.loss(action_mini, embed)
                    if cfg.use_reward_predictor:
                        # predict reward
            ...
            success_rate = np.mean(max3_success_rates)
            rewards = np.mean(max3_returns)
            L.log({'success_rate':success_rate, 'reward':np.mean(rewards), 'epoch':epoch, 'total_time':time.time() - time_start}, step=num_iterations, category="eval")
