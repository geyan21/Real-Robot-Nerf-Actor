...
import utils
import os
from color_jitter import ColorJitter, ColorJitterPairwise

places_dataloader = None
places_iter = None


def random_color_jitter_pairwise(x1, x2):
	"""Apply random color jitter to both images in a pair"""
	b,c,h,w = x1.shape
	transform = ColorJitterPairwise(
		brightness=0.4, 
		contrast=0.4,
		saturation=0.4, 
		hue=0.5,
	...
	x1, x2 = transform(x1.view(-1,3,h,w)/255., x2.view(-1,3,h,w)/255.)
	x1 = x1.view(b,c,h,w)*255.
	x2 = x2.view(b,c,h,w)*255.
	return x1, x2
