...
class PointCloudGenerator(object):
    ...
    @param max_bound: If not None, list len(3) containing largest x, y, and z
        values that will not be cropped
    """
    def __init__(self, sim, min_bound=None, max_bound=None, img_width=640, img_height=480):
        ...
        print(colored("[PointCloudGenerator] All Cameras: ", "green"), self.cam_names)
        self.used_cam_id = [1]
        self.used_cam_names = [self.cam_names[i] for i in self.used_cam_id]
        print(colored("[PointCloudGenerator] Used Cameras: ", "green"), self.used_cam_names)
    ...
    def generateCroppedPointCloud(self, save_img_dir=None):
        ...
        return np.array(combined_cloud.points)
