...
class _RenderWrapper(torch.nn.Module):
    ...
    def forward(self, rays, want_weights=False, extract_radience=False,ret_last_feat=False):
        if rays.shape[0] == 0:
            return (
        ...
        outputs = self.renderer(
            ...
            extract_radience=extract_radience, ret_last_feat=ret_last_feat
        )
        if extract_radience:
            return outputs
...
class NeRFEmbedRenderer(torch.nn.Module):
    ...
    def composite(self, model, rays, z_samp, coarse=True, sb=0):
        ...
        with profiler.record_function("renderer_composite"):
            ...
            if not self.regress_coord:
                ...
                return weights, rgb_final, embed_final, coord_final, depth_final  # make sure depth_final is the last one

    def forward(
        self, model, rays, want_weights=False, extract_radience=False, ret_last_feat=False
    ):
        """
        ...
        with profiler.record_function("renderer_forward"):
            ...
            if self.using_fine:
                ...
                if extract_radience: # output point radiance
                    radience = self.extract_radience(
                        model, rays, z_combine_sorted, coarse=False, sb=superbatch_size,ret_last_feat=ret_last_feat,
                    )
                    return radience
    ...
    @classmethod
    def from_conf(cls, conf, white_bkgd=False, lindisp=False, regress_coord=False, ret_last_feat=False, eval_batch_size=100000):
        return cls(
    ...
    def extract_radience(self, model, rays, z_samp, coarse=True, sb=0, ret_last_feat=False):
        """
        extract radience from the network
        ...
        with profiler.record_function("renderer_composite"):
            ...
            return points, rgbs, sigmas, embeds
