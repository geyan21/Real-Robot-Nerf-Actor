...
def add_gaussian_noise(x, scale=0.1):
...
class PixelNeRFEmbedNet(torch.nn.Module):
    def __init__(self, conf, stop_encoder_grad=False):
        ...
        # aug nerf
        self.use_input_aug = conf.get_bool("use_input_aug", False)
        self.use_layer_aug = conf.get_bool("use_layer_aug", False)
        self.use_output_aug = conf.get_bool("use_output_aug", False)
        self.aug_noise_scale = conf.get_float("aug_noise_scale", 0.0)


    def encode(self, images, poses, focal, z_bounds=None, c=None, language_embed=None):
        """
    ...
    def forward(self, xyz, coarse=True, viewdirs=None, far=False, ret_last_feat=False):
        ...
        with profiler.record_function("model_inference"):
            ...
            if self.use_input_aug and self.training:
                xyz = add_gaussian_noise(xyz, self.aug_noise_scale)
            ...
            if coarse or self.mlp_fine is None:
                mlp_output = self.mlp_coarse(
                    ...
                    language_embed=self.language_embed,
                    batch_size=SB
                )
            else:
            ...
            if self.use_output_aug and self.training:
                mlp_output = add_gaussian_noise(mlp_output, self.aug_noise_scale)
    ...
    def load_weights(self, args=None, model_path=None, opt_init=False, strict=True, device=None):
        """
