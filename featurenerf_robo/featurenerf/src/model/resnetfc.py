...
from .attention import LanguageInformedVisualAttention
...
class ResnetFC(nn.Module):
    def __init__(
        ...
        if self.use_language:
            self.VL_attention = LanguageInformedVisualAttention(language_dim=1024, visual_dim=512)
            print(colored("[ResnetFC] Using language informed visual attention", "red"))


    def forward(self, zx, combine_inner_dims=(1,), combine_index=None, dim_size=None, \
                ret_last_feat=False, language_embed=None, batch_size=None):
        """
        ...
        with profiler.record_function("resnetfc_infer"):
            ...
            if self.use_language: # visual-language attention
                z = z.reshape(batch_size, -1, z.shape[-1])
                z = self.VL_attention(visual_embed=z, language_embed=language_embed)
            ...
            for blkid in range(self.n_blocks):
                ...
                # if blkid == 0: # language informed visual feature
                #     x = x.reshape(batch_size, -1, x.shape[-1])
                #     lang_info = self.VL_attention(visual_embed=x, language_embed=language_embed)
                #     x = x + lang_info
    ...
    def from_conf(cls, conf, d_in, **kwargs):
        ...
        return cls(
            ...
            use_language=conf.get_bool("use_language", False),
            **kwargs
        )
