...
def apply_color_jitter(images, hue_range=0.1, saturation_range=0.1, brightness_range=0.1, contrast_range=0.1):
    # apply the same color jitter over batch of images
    hue_factor = np.random.uniform(-hue_range, hue_range)
    saturation_factor = np.random.uniform(1 - saturation_range, 1 + saturation_range)
