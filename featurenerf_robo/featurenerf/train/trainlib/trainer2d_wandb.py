...
class Trainer2DWandb:
    def __init__(self, net, net_aux, train_dataset, test_dataset, args, conf, device=None):
        self.args = args
        self.net = net
        self.net_aux = net_aux
        self.train_dataset = train_dataset
        self.test_dataset = test_dataset
        self.use_wandb = not args.not_use_wandb
        ...
        if self.use_wandb:
            wandb.init(project="nerf", group=self.exp_name, name=date, config=conf)

        self.fixed_test = hasattr(args, "fixed_test") and args.fixed_test

        # Currently only Adam supported
        self.optim = torch.optim.Adam(itertools.chain(net.parameters(), net_aux.parameters()), lr=args.lr)
        ...
        for epoch in range(self.num_epochs):
            ...
            for _ in range(self.num_epoch_repeats):
                for data in self.train_data_loader:
                    ...
                    if batch % self.print_interval == 0 and self.use_wandb:
                        print(
                            "E",
                            epoch,
                            "B",
                    ...
                    if batch % self.save_interval == 0 and (epoch > 0 or batch > 0) and self.use_wandb:
                        print("saving")
                        if self.managed_weight_saving:
