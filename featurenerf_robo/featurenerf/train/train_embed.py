...
def extra_args(parser):
    ...
    parser.add_argument(
        ...
        type=str,
        default="none",
        choices=["none", "dino", "clip"],
        help="Teacher model to use for feature training")
    return parser
...
dset, val_dset, _ = get_split_dataset(args.dataset_format, args.datadir, 
                ...
                #use_color_jitter=conf['data']['use_color_jitter'], 
                **extra_dataset_kwargs)
print(
...
class PixelNeRFTrainer(trainlib.TrainerWandb):
    def __init__(self):
        ...
        self.lambda_attn = conf.get_float("loss.lambda_attn", 0.1)
        print(
        ...
        self.teacher_model = conf.get_string("model.teacher_model", "none")
        self.use_color_jitter = conf.get_bool("data.use_color_jitter", False)
        if self.use_color_jitter and self.teacher_model == "dino":
            # the mode to recompute featmap
            from model import DINO
            pretrain_path = "/data/yanjieze/projects/nerf-ssl/ckpts/dino_deitsmall8_pretrain.pth"
            self.teacher_net = DINO(pretrain_path=pretrain_path).eval().cuda()
    
    def attn_crit(self, embed_fea, atten_cls):
        
        n_pow=1
        embed_fea_norm = F.normalize(embed_fea.pow(n_pow).mean(-1).view(embed_fea.size(0), -1))
        att_norm = F.normalize(atten_cls.pow(n_pow).mean(-1).view(atten_cls.size(0), -1))
        
        # Loss now is just the p2-norm of the normalized attention maps!
        loss = (embed_fea_norm - att_norm).pow(2).mean()
        return loss
    ...
    def calc_losses(self, data, is_train=True, global_step=0):
        ...
        # language_embed = data['sentence_emb'].to(device=device)
        language_embed = None
        ...
        if is_train:
            ...
            all_cls_attn = data["cls_attn"].to(device=device)  # (SB, NV, 6, H // 8, W // 8)
            #pdb.set_trace()
            if self.use_color_jitter and self.teacher_model == "dino":
                # since we use color jitter in img,
                # we need to recompute feats
                SB, B, C, H, W = all_images.shape
                all_feats = self.teacher_net(all_images.view(SB * B, C, H, W))
                ...
                all_feats = all_feats.view(SB, B, -1, H, W).to(device=device)
        ...
        for obj_idx in range(SB):
            ...
            # image grid, for debug
            # pix = torch.stack(torch.meshgrid(torch.arange(H), torch.arange(W)), dim=-1)
            # pix = torch.cat((torch.zeros_like(pix[..., :1]), pix), dim=-1)
            # pix = pix.reshape(-1, 3)
            # pix_inds = pix[..., 0] * H * W + pix[..., 1] * W + pix[..., 2]
            # import vlab
            ...
            if is_train:
                ...
                grid_n1to1 = (grid.float() / torch.tensor([H, W]).float()) * 2. - 1.
                ...
                attn_gt = F.grid_sample(attn, grid_n1to1.to(device), align_corners=False)
                attn_gt = attn_gt[pix[..., 0], :, 0, torch.arange(pix.shape[0])]
                all_feat_gt.append(feat_gt)
                all_attn_gt.append(attn_gt)
        ...
        if is_train:
            ...
            all_attn_gt = torch.stack(all_attn_gt)  # (SB, ray_batch_size, 6)
            #pdb.set_trace()
            if self.mask_feat:
                ...
                all_attn_gt = all_attn_gt * all_mask_gt[..., None]
        ...
        if is_train:
            ...
            if self.lambda_attn > 0:
                attn_loss = self.attn_crit(coarse.embed, all_attn_gt)           
                loss_dict["ac"] = attn_loss.item() * self.lambda_attn
        if using_fine:
            ...
            if is_train:
                ...
                if self.lambda_attn > 0:
                    fine_attn_loss = self.attn_crit(fine.embed, all_attn_gt)
                    attn_loss = (attn_loss + fine_attn_loss) * self.lambda_attn
                    loss_dict["af"] = fine_attn_loss.item() * self.lambda_attn
                if self.lambda_coord > 0:
    ...
    def vis_step(self, data, global_step, idx=None):
        ...
        #language_embed = data["sentence_emb"][batch_idx : batch_idx + 1]
        language_embed = None
        ...
        with torch.no_grad():
            ...
            net.encode(
                ...
                # language_embed=language_embed.to(device=device),
            )
            ...
            embed_coarse_np = coarse.embed[0].cpu().numpy().reshape(H, W, -1).mean(-1)
